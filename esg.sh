...
you can activate your MainNet config with "esg.sh switch MainNet" 
...
function upgrade_conf () {
    ...
    then
        ...
        BRS="${BRS//esg\.rebroadcastPeers/P2P.rebroadcastTo}"
        BRS="${BRS//esg\.connectWellKnownFirst/P2P.NumBootstrapConnections}"
        ...
        BRS="${BRS//esg\.rebroadcastAfter/P2P.rebroadcastTxAfter}"
        BRS="${BRS//esg\.rebroadcastEvery/P2P.rebroadcastTxEvery}"
        ...
        BRS="${BRS//esg\.mockMining/DEV.mockMining}"
        ...
        # GPU-related params
        BRS="${BRS//esg\.oclVerify/GPU.Acceleration}"
        BRS="${BRS//esg\.oclAuto/GPU.AutoDetect}"
        BRS="${BRS//esg\.oclPlatform/GPU.PlatformIdx}"
        BRS="${BRS//esg\.oclDevice/GPU.DeviceIdx}"
        BRS="${BRS//esg\.oclMemPercent/GPU.MemPercent}"
        BRS="${BRS//esg\.oclHashesPerEnqueue/GPU.HashesPerBatch}"
...
function create_brs_db {
    ...
    echo "[+] Creating esg wallet db ($P_DATA)..."
...
function exists_or_get {
    if [ -f $1 ]; then
        ...
        wget https://download.cryptoguru.org/esg/wallet/$1
    fi
}
...
if [[ $# -gt 0 ]] ; then
    case "$MY_CMD" in
        ...
        "compile")
            ...
            if hash mvn 2>/dev/null; then
                ...
                echo Nevertheless, now you can start the wallet with ./esg.sh
            else
            ...
            ;;
        "h2shell")
            java -cp esg.jar org.h2.tools.Shell
            ;;
        "import")
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                if [[ $MY_ARG == "mariadb" ]]; then
                    ...
                    if exists_or_get brs.h2.zip ; then
                        mkdir -p "$MY_DIR/esg_db"
                        rm -f esg_db/esg.trace.db
                        if unzip brs.h2.zip ; then
                            if mv esg.mv.db "$MY_DIR/esg_db"; then
    ...
    java $BRS_DEVSTART -cp esg.jar:conf brs.Esg
fi
