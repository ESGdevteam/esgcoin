...
public class AccountServiceImpl implements AccountService {
  ...
  private final LongKeyFactory<Account> accountEsgKeyFactory;
  ...
  public AccountServiceImpl(AccountStore accountStore, AssetTransferStore assetTransferStore) {
    ...
    this.accountEsgKeyFactory = accountStore.getAccountKeyFactory();
  ...
  public Account getAccount(long id) {
    return id == 0 ? null : accountTable.get(accountEsgKeyFactory.newKey(id));
  }

  @Override
  public Account getAccount(long id, int height) {
    return id == 0 ? null : accountTable.get(accountEsgKeyFactory.newKey(id), height);
  }

  @Override
  public Account getAccount(byte[] publicKey) {
    final Account account = accountTable.get(accountEsgKeyFactory.newKey(getId(publicKey)));
  ...
  public Account getOrAddAccount(long id) {
    Account account = accountTable.get(accountEsgKeyFactory.newKey(id));
    if (account == null) {
  ...
  public void addToAssetBalanceQNT(Account account, long assetId, long quantityQNT) {
    ...
    EsgKey newKey = accountAssetKeyFactory.newKey(account.getId(), assetId);
  ...
  public void setRewardRecipientAssignment(Account account, Long recipient) {
    int currentHeight = Esg.getBlockchain().getLastBlock().getHeight();
    RewardRecipientAssignment assignment = getRewardRecipientAssignment(account.getId());
    if (assignment == null) {
      EsgKey esgKey = rewardRecipientAssignmentKeyFactory.newKey(account.getId());
      assignment = new RewardRecipientAssignment(account.getId(), account.getId(), recipient, (int) (currentHeight + Constants.ESG_REWARD_RECIPIENT_ASSIGNMENT_WAIT_TIME), esgKey);
    } else {
      assignment.setRecipient(recipient, (int) (currentHeight + Constants.ESG_REWARD_RECIPIENT_ASSIGNMENT_WAIT_TIME));
    }
  ...
  public long getUnconfirmedAssetBalanceQNT(Account account, long assetId) {
    EsgKey newKey = Esg.getStores().getAccountStore().getAccountAssetKeyFactory().newKey(account.getId(), assetId);
