...
public class AtConstants {
    ...
    private AtConstants() {
        ...
        STEP_FEE.put((short) 1, Constants.ONE_ESG / 10L);
        MAX_STEPS.put((short) 1, 2000L);
        API_STEP_MULTIPLIER.put((short) 1, 10L);

        COST_PER_PAGE.put((short) 1, Constants.ONE_ESG);
        ...
        MAX_PAYLOAD_FOR_BLOCK.put((short) 1, (Esg.getFluxCapacitor().getValue(FluxValues.MAX_PAYLOAD_LENGTH)) / 2L); //use at max half size of the block.
        ...
        MAX_PAYLOAD_FOR_BLOCK.put((short) 2, (Esg.getFluxCapacitor().getValue(FluxValues.MAX_PAYLOAD_LENGTH)) / 2L); //use at max half size of the block.
    ...
    public short atVersion(int blockHeight) {
        return Esg.getFluxCapacitor().getValue(FluxValues.AT_VERSION, blockHeight);
    }
